package com.airta.platform.engine.util;
...
//        redisClient.KeyOperate();
    ...
    public void storeObject(String key, PodSessionPool obj) {
        ...
        jedis.set(key,json);
        logger.info("## Successfully stored object to pool {}", key);
    }

    public PodSessionPool readObject(String key) {

        if(jedis.exists(key)) {
            Gson gson = new Gson();
            String json = jedis.get(key);
            PodSessionPool object=gson.fromJson(json, PodSessionPool.class);
            logger.info("## Successfully fetched object with key {}", key);
            return object;
        } else {
            logger.warn("## cannot read the object with key {}", key);
            return null;
        }
    }
    ...
    public void KeyOperate() {
        ...
        // clear data
        System.out.println("clearing the data：" + jedis.flushDB());
        // check if key exist
        System.out.println("check key999 exist：" + jedis.exists("key999"));
